...
class BaseModel(LightningModule):
    ...
    def on_before_optimizer_step(self, optimizer):
        ...
        total_norm_squared = torch.tensor(0.0, device=next(self.parameters()).device)
        for p in self.parameters():
            if p.grad is not None:
                param_norm = p.grad.data.norm(2)
                total_norm_squared += param_norm ** 2
        total_norm = total_norm_squared ** (1. / 2)


        self.log('grad_2.0_norm', total_norm, on_step=True, on_epoch=False)
    ...
    def norm(self, x, stats):
        ...
        if self.norm_type == "standardize":
            mean = stats['mean'].to(target_device)
            std = stats['std'].to(target_device)
            return (x - mean) / (2*(std + 1e-5))
        elif self.norm_type == "min_max":
            max = stats['max'].to(target_device)
            min = stats['min'].to(target_device)
    ...
    def allsplit_epoch_end(self, split: str):
        ...
        if split == 'val':
            ...
            is_vae_stage = hasattr(self, 'stage') and self.stage == 'vae'
            ...
            if not is_vae_stage:
                ...
                if self.trainer.world_size > 1:
                    ...
                    gathered_outputs = [None] * self.trainer.world_size
                    ...
                    if self.global_rank == 0:
                        ...
                        for key in self.validation_step_outputs.keys():
                            merged_outputs[key] = {}
                            for gpu_output in gathered_outputs:
                                if key in gpu_output:
                                    merged_outputs[key].update(gpu_output[key])
                        self.validation_step_outputs = merged_outputs
                ...
                if self.global_rank == 0:
                    for guid_comb, samples_gen in self.validation_step_outputs.items():
                        ...
                        if not isinstance(samples_gen, dict):

                            continue



                        results = retrieval(samples_gen, evaluate_gt=False, compute_distance=False)
                        metr_batch, metr_full, fid_metrics, distance_metric = results[:4]
                        ...
                        if fid_metrics:
                            dict_to_log_metrs.update({
                                f'metrics_{guid_comb}/{metric_name}': float(metric_value)
                                for metric_name, metric_value in fid_metrics.items()
                            })
                        self.log_dict(dict_to_log_metrs, sync_dist=True, rank_zero_only=True)
    ...
    def on_validation_epoch_end(self):
        ...
        if hasattr(self, 'mr_metrics') and self.mr_metrics is not None:
            if not self.trainer.sanity_checking:


                if self.mr_metrics.has_updated():
                    metrics_dict = self.mr_metrics.compute(sanity_flag=self.trainer.sanity_checking)


                    for metric_name, metric_value in metrics_dict.items():
                        self.log(
                            f"Metrics/MR/{metric_name}",
                            metric_value.item() if torch.is_tensor(metric_value) else metric_value,
                            on_step=False,
                            on_epoch=True,
                            sync_dist=True,
                            rank_zero_only=True
