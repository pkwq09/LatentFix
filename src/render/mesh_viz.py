...
def render_motion(renderer: HeadlessRenderer, datum: dict,
    ...
    ref_global_orient = None
    ref_body_transl = None
    ...
    for iid, mesh_seq in enumerate(datum):
        ...
        if ref_global_orient is None:
            ref_global_orient = global_orient
            ref_body_transl = body_transl
        sys.stdout.flush()
    ...
    mid_idx = ref_global_orient.shape[0] // 2
    R_z = get_z_rot(ref_global_orient[mid_idx], in_format='aa')
    heading = -R_z[:, 1]
    xy_facing = ref_body_transl[mid_idx] + heading*2.5
