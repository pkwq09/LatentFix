...
#   global_rot[child] = global_rot[parent] @ local_rot[child]
...
class MotionFixDataModule(BASEDataModule):
    ...
    def __init__(self,
        ...
        slice_test = int(1 * len(splits['test']))
    ...
    def _canonica_facefront(self, rotations, translation):
        ...
        rots_motion_rotmat = transform_body_pose(
            rots_motion.reshape(datum_len, -1, 3), 'aa->rot'
        )

        orient_R_can, trans_can = canonicalize_rotations(
            rots_motion_rotmat[:, 0],
            trans_motion
        )
        ...
        rots_motion_aa_can = transform_body_pose(
            rots_motion_rotmat_can, 'rot->aa'
        )
    ...
    def calculate_feature_stats(self, dataset: MotionFixDataset):
        ...
        if not exists(stat_path):
            ...
            feature_dict = {
                ...
                for name in feature_names if '_target' not in name
            }
            ...
            feature_dict = {
                name: torch.cat(feature_dict[name], dim=0).float()
