"""MotionFix training entry (Hydra + PyTorch Lightning).

Uses Hydra for config and Lightning for training (including classifier-free guidance).
...
def _train(cfg: DictConfig):
    """Hydra-wrapped training entry: optional resume, then calls train()."""
    ckpt_ft = None
...
def train(cfg: DictConfig, ckpt_ft: Optional[str] = None) -> None:
    """Set up data, model, callbacks, and run Lightning training."""
    import os
    import torch
    import socket
    ...
    if isinstance(devices_cfg, (list, tuple, ListConfig)):
        ...
        try:
            num_devices = int(devices_cfg)
        except (TypeError, ValueError):
            logger.warning(f"Could not parse devices={devices_cfg}; defaulting to single device.")
            num_devices = 1
